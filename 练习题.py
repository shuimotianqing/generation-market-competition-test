...
class TrueFalseQuizApp:
    def __init__(self, root, file_path):
        self.root = root
        self.root.title("判断题测试")
        
        # 设置窗口大小为固定大小，并拉大高度
        self.root.geometry("400x500")  # 增大了窗口的高度
        
        # 读取 Excel 题库（选择名为“判断题”的工作簿）
        self.df = pd.read_excel(file_path, sheet_name="判断题")
        self.questions = self.df['问题'].tolist()
        
        # 将答案列中的 1 或 2 转换为“正确”或“错误”，处理NaN或空值
        self.answers = self.df['答案'].apply(lambda x: '正确' if x == 1 else ('错误' if x == 2 else '错误')).tolist()  
        ...
        self.option_true = tk.Radiobutton(root, text="正确", variable=self.var, value="正确", command=self.check_answer, anchor="w")
        self.option_false = tk.Radiobutton(root, text="错误", variable=self.var, value="错误", command=self.check_answer, anchor="w")
    ...
    def check_answer(self):
        ...
        correct_answer = self.answers[self.current_question].strip()  # 统一为"正确"或"错误"
    ...
    def update_question(self):
        ...
        # 更新选项文本
        self.option_true.config(text="正确")  # 这里不再显示0或1，直接显示文字
        self.option_false.config(text="错误")  # 这里不再显示0或1，直接显示文字
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    app = TrueFalseQuizApp(root, file_path)
    root.mainloop()
